...
## Статус: DONE — добавлен выбор иконки воркспейса

### Что сделано
- Добавлен IPC-канал выбора файла изображения через системный диалог.
- Кнопка Change открывает диалог, показывает имя выбранного файла и сохраняет путь в store по ключу воркспейса.
- Исправлена передача пути репозитория при переходе из WorkspacePage.

### Acceptance criteria
- [x] Клик по кнопке «Change» открывает системный диалог выбора файла.
- [x] Выбранный файл отображается в кнопке и сохраняется в store.
- [x] `npm run tsc` + `eslint` проходят без ошибок.
- [x] FSD-слои соблюдены (IPC-канал, shared API и entity hook разделены).

### Заметки для ревьюера
- Иконка сохраняется как путь к выбранному файлу; копирование файла в директорию воркспейса не выполняется в MVP.
